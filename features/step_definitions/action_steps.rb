...
Given /^I have a personality of:$/ do |table|
  @entities = {}
  @self = Reactive::Self.new(Reactive::Personality.new(table.rows_hash))
end
...
Given /^I have feelings towards (\w+) currently as:$/ do |name, table|
  @entities[name].feelings_toward = Reactive::Feelings.new(table.rows_hash)
end

Given /^(\w+) performs the action Murder on (\w+)$/ do |name_src, name_dst|
  @action = Reactive::Actions::Murder.new(:source       => @entities[name_src],
...
Then /^the action should have the effect on (\w+) as:$/ do |name, table|
  ...
  effect.feelings_change[@entities[name]].should == Reactive::Feelings.new(table.rows_hash).to_hash
end

Then /^my feelings toward (\w+) should be:$/ do |name, table|
  expected_feelings = Reactive::Feelings.new(table.rows_hash)

  @entities[name].feelings_toward.should == expected_feelings
end

Then /^my sentiment towards (\w+) should be (.+)$/ do |name, score|
  @entities[name].sentiment_toward.should == Reactive::Sentiment.new(score.to_f)
end
